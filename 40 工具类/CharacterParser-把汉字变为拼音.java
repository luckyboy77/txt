...
public class CharacterParser {
	...
	/**
	 * 词组解析之第一个汉字全拼，之后的取首字母，例如：徐朋飞--->xupf
	 * @param chs
	 * @return
	 */
	public String getSelling1(String chs) {
		String key, key1, value, value1 = null;
		buffer = new StringBuilder();
		for(int i = 0;i<1;i++){
			key1 = chs.substring(0, 1);
			if (key1.getBytes().length >= 2) {
				value1 = (String) convert(key1);
				if (value1 == null) {
					value1 = "unknown";
				}
			} else {
	...
	public static void main(String[] args) {
		...
		String name = "张三";
		if(name.length()>=3){
			System.out.println(parser.getSelling1(name));
		}else{
			System.out.println(parser.getSelling(name));
		}		
	}
}